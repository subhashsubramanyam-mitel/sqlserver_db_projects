CREATE VIEW [analytics].[vw_ucaas_opps_cq_open] AS
...
case
		...
		when cust2.NAME like '%PLANETONE COMMUNICATIONS%' then 'Master Agent'
		when cust2.NAME like '%SANDLER%' then 'Master Agent'
		when cust2.NAME like '%Sandler%' then 'Master Agent'
		when cust2.NAME like '%TCG (TELECOM CONSULTING GROUP%' then 'Master Agent'
...
case
		when cast(dbo.OPPORTUNITY.NumOfProfiles AS numeric) >= 250 then '250+'
		when cast(dbo.OPPORTUNITY.NumOfProfiles AS numeric) >= 100 then '101-250'
		when cast(dbo.OPPORTUNITY.NumOfProfiles AS numeric) >= 50 then '51-100'
		else '0-50'
end as 'seat_segment',
...
where	dbo.OPPORTUNITY.CloseDate >= '2020-01-01 00:00:00' and dbo.OPPORTUNITY.CloseDate <= '2020-03-31 00:00:00'
		--and dbo.OPPORTUNITY.IsClosed = 'true'
		and dbo.OPPORTUNITY.EstimatedMRR > 0
		and dbo.OPPORTUNITY.isSecondary = 'true'
		and dbo.OPPORTUNITY.Stage in ('Decision','Defining','Investigating','Validating','Evaluating')
		and (dbo.OPPORTUNITY.SubType is null or dbo.OPPORTUNITY.SubType not in ('Flex CC and/or SIP','Cross-Sell','Migrate Call Conductor to MiCloud Connect','Migrate MiCB v1.0 to MiCloud Connect','Migrate MiCB v2.0 to MiCloud Connect','Migrate SHV to MiCloud Connect','Additions to Call Conductor','Additions to Private and Other Cloud','MiCloud EngageCC','MiCloud Flex','Other','Private and Other Cloud'))
		and dbo.OPPORTUNITY.Type in ('New Business - Initial Sale','New Business-Initial Sale','Existing Customer - Add on Sale')
		and dbo.OPPORTUNITY.ProductInterest in ('Cloud Service','Cloud Service - Connect','MiCloud Connect CX','UCaaS - Other')
		...
		--and dbo.OPPORTUNITY.Name not like '%- CSM%'
		--and dbo.OPPORTUNITY.Name not like '% CSM%'
		and ((dbo.SFDC_TERRITORY.Region in ('USA','Canada') and dbo.TABLEAU_B_SFDC_OPP_PRODUCTS.ProductName in ('MiCloud Connect','Monthly Recurring Revenue'))
