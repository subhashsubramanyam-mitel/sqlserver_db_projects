...
CREATE View [dbo].[V_SFDC_TASKFEED_INFO_Arch] as
...
,o.OpportunityNumber
,a.[TF Color Number]
,null [TF Tsk Project Start Date Start Date]
,null [TF Tsk Project Start Date Sch End Date]
,null [TF Tsk Project Start Date Complete Date]
,null [TF Tsk Project Start Date Status]
...
,null 	[TF Tsk Port In Research Start Date]
,null 	 [TF Tsk Port In Research End Date]
,null 	 [TF Tsk Port In Research Complete Date]
,null 	[TF Tsk Port In Research Status]
,null 	[TF Tsk Order New Numbers Start Date]
,null 	[TF Tsk Order New Numbers End Date]
,null 	 [TF Tsk Order New Numbers Complete Date]
,null 	[TF Tsk Order New Numbers Status]
,null 	[TF Tsk Order Hardware Start Date]
,null 	 [TF Tsk Order Hardware End Date]
,null 	[TF Tsk Order Hardware Complete Date]
,null 	 [TF Tsk Order Hardware Status]
,null 	[TF Tsk Port Submitted Start Date]
,null 	[TF Tsk Port Submitted End Date]
,null 	 [TF Tsk Port Submitted Complete Date]
,null 	[TF Tsk Port Submitted Status]
,null 	[TF Tsk System Programming Start Date]
,null 	 [TF Tsk System Programming End Date]
,null 	[TF Tsk System Programming Complete Date]
,null 	 [TF Tsk System Programming Status]
,null 	[TF Tsk Ready To Test Start Date]
,null 	 [TF Tsk Ready To Test End Date]
,null 	[TF Tsk Ready To Test Complete Date]
,null 	 [TF Tsk Ready To Test Status]
,null 	[TF Tsk Port Scheduled Start Date]
,null 	 [TF Tsk Port Scheduled End Date]
,null 	[TF Tsk Port Scheduled Complete Date]
,null 	 [TF Tsk Port Scheduled Status]
,null 	[TF Tsk Pre Go Live Testing Start Date]
,null 	[TF Tsk Pre Go Live Testing End Date]
,null 	 [TF Tsk Pre Go Live Testing Compelte Date]
,null 	[TF Tsk Pre Go Live Testing Status]
...
FROM            V_TABLEAU_TASKFEED_BOARD AS a 
            ...
            left outer JOIN   Users AS b ON a.OwnerId = b.ID
				...
				left join (SELECT    
					  ...
					  FROM [dbo].[V_TABLEAU_TASKFEED_TASK] (nolock)
					where TaskName  in ('Transition to Support and CSM' , 'Transition to Support/CSM')
				...
				left join (SELECT    
					...
					) atl on a.BoardSfdcId = atl.BoardId and atl.rn = 1
where 
...
SELECT    
					  ...
					  FROM  [V_TABLEAU_TASKFEED_TASK] (nolock)
					where TaskName in ('Ready to test' , 'Ready to Test Notice Sent')
...
SELECT    
					  ...
					  FROM  [V_TABLEAU_TASKFEED_TASK] (nolock)
					where TaskName  in ('Pre-Go Live Testing' , 'CSE Follow Up')
