/*
CREATE VIEW [dbo].[V_QMS_RESELLERLIST] as
...
/*
		case when   RTRIM(REPLACE(REPLACE(a.Phone, CHAR(13), ''), CHAR(10), ''))  is not null then
		   '"' + CASE WHEN CHARINDEX(CHAR(10), a.Phone) = '0' THEN LEFT(a.Phone,16) ELSE SUBSTRING(a.Phone, 0, 
...
/*
LEFT OUTER JOIN
				(SELECT distinct AccountSTID AS ST_ID, FName, LName, FName + ' ' + LName AS Name,
					Email,
                    CASE WHEN CHARINDEX(CHAR(10), WorkPhone) = '0' THEN WorkPhone ELSE SUBSTRING(WorkPhone, 0, 
                    CHARINDEX(CHAR(10), WorkPhone)) END AS Phone
						FROM		CONTACTS (nolock)
						WHERE      SfdcId=PrimaryContactID) it ON a.ImpactNumber = it.ST_ID
*/
where b.TotalDisc<>'0' and isnumeric(a.ImpactNumber) = 1
*/
