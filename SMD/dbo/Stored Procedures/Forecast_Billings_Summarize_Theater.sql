...
-- =============================================
CREATE PROCEDURE [dbo].[Forecast_Billings_Summarize_Theater]
AS
BEGIN
...
DECLARE @Forecast_Billings_theater table
(
[WeekBucket] int,
...
insert into @Forecast_Billings_theater
select 
[WeekBucket]
 ,[Fiscal Quarter]
,sc.Date
...
select @currentQuarter = 
 a.[Fiscal Quarter]
from @Forecast_Billings_theater a
where
...
-------------------
select @CurrentDay = max(vb.date) from   @Forecast_Billings_theater vb
...
(select max(weekbucket) from  @Forecast_Billings_theater  where [SumOfNetBookedUSD] is not null and [Fiscal Quarter] = @currentQuarter and date = @CurrentDay )
...
select sum([SumOfNetBookedUSD]), Theater from  @Forecast_Billings_theater   where [Fiscal Quarter] = @PreviousQuarter
and date <=
(
Select date from (
...
select sum([SumOfNetBookedUSD]), Theater from @Forecast_Billings_theater  where [Fiscal Quarter] = @PreviousYear
and date <=
(
Select date from (
select rank() OVER (Order by date) as rank, WeekBucket,date,IsBusinessDay from salescalendar
...
SELECT a.[Fiscal Quarter],a.[WeekBucket],sum(a.[SumOfNetBookedUSD]) 'WeeklyBooked',
 (SELECT SUM(b.[SumOfNetBookedUSD])
                       FROM  @Forecast_Billings_theater  b
...
insert into [Forecast_Billings_Summary_Theater]
([Fiscal Quarter]
           ,[WeekBucket]
           ,[Total Booked]
		   ,[Theater]
...
---------------------------------
Update [Forecast_Billings_Summary_Theater]
set [Total Booked] = p.PrevQuarterBooked
from @PrevQuarterBooked p
join [Forecast_Billings_Summary_Theater] f
