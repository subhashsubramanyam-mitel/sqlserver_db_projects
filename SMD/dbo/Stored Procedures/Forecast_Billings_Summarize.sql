...
-- =============================================
CREATE PROCEDURE [dbo].[Forecast_Billings_Summarize]
AS
BEGIN
...
-------------------
select @CurrentDay = max(vb.date) from   [V_Forecast_Billings] vb
...
(select max(weekbucket) from  [V_Forecast_Billings]  where [SumOfNetBookedUSD] is not null and [Fiscal Quarter] = @currentQuarter and date = @CurrentDay )
...
select @PrevQuarterBooked = sum([SumOfNetBookedUSD]) from  [V_Forecast_Billings]   where [Fiscal Quarter] = @PreviousQuarter
and date <=
(
Select date from (
...
select @PrevYearBooked = sum([SumOfNetBookedUSD]) from [V_Forecast_Billings]  where [Fiscal Quarter] = @PreviousYear
and date <=
(
Select date from (
select rank() OVER (Order by date) as rank, WeekBucket,date,IsBusinessDay from salescalendar
...
-- MW 10042017 doing truncate here
truncate table [Forecast_Billings_Summary]
 

insert into [Forecast_Billings_Summary]
([Fiscal Quarter]
           ,[WeekBucket]
           ,[Total Booked]
    )
...
---------------------------------
Update [Forecast_Billings_Summary]
...
where 
sc.WeekBucket = @currentWeekBucket and sc.WeekBucket <> 13   -- MW ??????????????
