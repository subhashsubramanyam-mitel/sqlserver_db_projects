...
CREATE View [dbo].[V_SFDC_INVOICE] as
...
SELECT  --   top 5
			...
			InvoiceDate, 
			--FIX FOR BUNDLE SKUS
			CASE WHEN isnumeric(SKU) = 0 then null 
			WHEN len(SKU) >5 then null -- MW 12052015  some crazy skus not in SFDC
			else SKU END as SKU, 
			RevType, 
			CurrencyCode,
			Status,
...
UNION ALL
			...
			--FIX FOR BUNDLE SKUS
			CASE 
			WHEN isnumeric(a.SKU) = 0 then null 
			WHEN len(a.SKU) >5 then null -- MW 12052015  some crazy skus not in SFDC
			else SKU END as SKU, 
			a.RevType, 
			a.CurrencyCode,
			a.Status,
