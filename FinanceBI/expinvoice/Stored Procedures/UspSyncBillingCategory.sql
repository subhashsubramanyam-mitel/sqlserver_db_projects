...
-- Change Log: 2013-10-30 -- A separate version for Biling database
-- =============================================
CREATE PROCEDURE [expinvoice].[UspSyncBillingCategory] 
...
BEGIN
	...
	SELECT 
		...
		Description as Name
	FROM (
		select distinct Li.Description from M5DB.Billing.mock.billing_InvoiceLineItem LI
		left join enum.BillingCategory BC on BC.Name = LI.Description
		where BC.Name is null 
		and LI.DateGenerated >= @lastSyncUTC
		)foo
