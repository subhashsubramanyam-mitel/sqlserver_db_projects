...
-- =============================================
CREATE PROCEDURE [rollup].[UspComputeProfitability_Oct26]
	@svcMonth Datetime
AS
BEGIN
...
update CS
	set		
		CostPerSupportCase = case when (NumSupportCasesRegular+NumSupportCasesNoAccount) > 0 
			then Support.amt / ((NumSupportCasesRegular+NumSupportCasesNoAccount)) else 0.0 end 
		...
		--      [(Rolling 3 mo avg ‘NOC – Monitoring’ ) + (25%* ‘NOC – Connectivity’) / Number of Circuits
		,CostAccessOvhdPerCircuit = case when NumCircuits >0 then (NocMonitoring.amt + 0.25 * NocConnectivity.amt)/NumCircuits end 
from rollup.CommonStats CS
...
from (
	select M5dbAccountId AccountId, 
	SUM(case when Category in ( 'Support', 'Support/NOC') and caseType='Other' THEN 1 ELSE 0 END) * Max(Com.CostPerSupportCase)/3 CostSupport,
	SUM(case when category in ( 'NOC', 'Support/NOC') and caseType='Other' THEN 1 ELSE 0 END) * Max(Com.CostPerNocSupportCase)/3 CostNoc
	 from sfdc.VwSkyCase SC
	 inner join (select * from rollup.CommonStats where  SvcMonth = @svcMonth) Com on 1=1
	 where SC.SfdcCreateDate between @3mBegin and @3mEnd
