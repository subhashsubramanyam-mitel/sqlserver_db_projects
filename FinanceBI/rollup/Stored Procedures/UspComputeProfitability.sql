...
-- Change Log:  Moving to actual Partner Commission rate -- TODO, later move to fine grain PC 
--         2016-08-08 Tarak, changed definition of Access Ovhd per Andrew's email to:
--                      [(Rolling 3 mo avg ‘NOC – Monitoring’ ) + (25%* ‘NOC – Connectivity’) / Number of Circuits
--         2016-08-22 Tarak, revised back to include Tunnel, router, etc in 210 category
--         2018-07-10 Taram, revised to take into account null support case cost
-- =============================================
CREATE PROCEDURE [rollup].[UspComputeProfitability]
	@svcMonth Datetime
AS
BEGIN
...
inner join company.VwAccount AA on AA.[Ac Id] = TA.AccountId
where TA. SvcMonth = @svcMonth and AA.IsBillable = 1

-- NOC, Support
-- declare @svcMonth date = '2014-11-01';
	update TA	SET
		NumSupportCasesRegular = coalesce(SS.SupReg,0)/3,  -- per month
		NumNocCasesRegular = coalesce(SS.NSupReg,0)/3  -- per month
	from rollup.TmpAccountStats TA
	inner join 
		(select M5dbAccountId AccountId, 
			SUM(case when Category in ( 'Support', 'Support/NOC') and caseType='Other'  THEN 1 ELSE 0 END) SupReg,
			SUM(case when SC.CaseOwnerRole like '%NOC%' and caseType='Other' THEN 1 ELSE 0 END) NSupReg
...
inner join 
		(select 
			 SUM(CASE WHEN  SC.HasAccount = 1 and 
				(SC.Category in( 'Support', 'Support/NOC')) THEN 1 ELSE 0 END) NumSupportCasesRegular,
 			 ...
 			 SUM(CASE WHEN SC.Category in( 'Support', 'Support/NOC')  and SC.HasAccount = 0 
												 THEN 1 ELSE 0 END) NumSupportCasesNoAccount,		 
			 SUM(CASE WHEN SC.CaseOwnerRole like '%NOC%' THEN 1 ELSE 0 END) NumNocCasesTotal,
			 SUM(CASE WHEN SC.CaseOwnerRole like '%NOC%'and SC.HasAccount = 1
						and SC.CaseType = 'Outage' THEN 1 ELSE 0 END) NumNocCasesOutage,	
			 SUM(CASE WHEN SC.CaseOwnerRole like '%NOC%' and SC.HasAccount = 0 
						 THEN 1 ELSE 0 END) NumNocCasesNoAccount
		from sfdc.VwSkyCase SC
		 where SC.SfdcCreateDate  between @3mBegin and @3mEnd
...
update CS
	set		
		CostPerSupportCase = coalesce(case when (NumSupportCasesRegular+NumSupportCasesNoAccount) > 0 
			then Support.amt / ((NumSupportCasesRegular+NumSupportCasesNoAccount)) else 0.0 end, 0.0) 
		,CostPerNocSupportCase = coalesce(case when NumNocCasesTotal > 0 then  NocSupport.amt / (NumNocCasesTotal) else 0.0 end , 0.0) 
		,CostPerNocConnectivityMonitoringCase =coalesce(case when NumCircuits > 0 then  NocMonitoring.amt / NumCircuits else 0.0 end, 0.0) 
		,CostPerProfile = coalesce(case when NumProfiles > 0 then ( Networks.amt + NocProfiles.amt + Depreciation.amt)/NumProfiles else 0.0 end, 0.0) 
		,CostProfileInfraPerProfile = coalesce(case when NumProfiles > 0 then CostProfileInfra/NumProfiles else 0.0 end, 0.0) 
		,CostAccessInfraPerCircuit = coalesce(case when NumCircuits > 0 then  CostAccessInfra/NumCircuits else 0.0 end, 0.0) 
		,CostAcctMgmtPerProfile = coalesce(case when NumProfiles > 0 then  AcctMgmt.amt/NumProfiles else 0.0 end, 0.0) 
		-- below lineadded on 2016-08-08 Tarak, changed definition of Access Ovhd per Andrew's email to:
		--      [(Rolling 3 mo avg ‘NOC – Monitoring’ ) + (25%* ‘NOC – Connectivity’) / Number of Circuits
		,CostAccessOvhdPerCircuit = coalesce(case when NumCircuits >0 then (NocMonitoring.amt + 0.25 * NocConnectivity.amt)/NumCircuits end, 0.0)  
from rollup.CommonStats CS
...
--210	Access Ovhd	Access	NPFC 2016-08-08 moved to FCC+Circuits then 2016-089-22 moved back 
...
Select NPFC.AccountId, @svcMonth, NT.NPFCtype, 
	CASE when NT.NPFCtype = 203 THEN NPFC.RentalNPFC
		 when NT.NPFCtype = 210 THEN NPFC.AccessNPFC -- 2016-08-08 MOVED to and then back 2016-08-22 from FCC+Circuits
...
from (
	...
	-- 2016-08-22 MOVED back FCC+Circuits
	-1 * SUM (Case WHEN [Prod Category] = 'Access' and [Class Leaf Name] like '%t1%' THEN -1*AP.Quantity *Com.CostAccessOvhdPerCircuit -- For T1s get avg cost
	               WHEN [Prod Category] = 'Access' THEN NPFC ELSE 0.0 END +
				Case WHEN [Prod Category] = 'Applications' and P.[Class Leaf Name]='Internet'
					 THEN NPFC ELSE 0.0 END) AccessNPFC,
	-1 * SUM (AP.NPFC - Case WHEN [Prod Category] = 'Access' and [Class Leaf Name] like '%t1%' THEN 0 -- ot coming from NPFC		
							WHEN [Prod Category] = 'Access' THEN NPFC ELSE 0.0 END 
	...
	Where (NPFC > 0 or P.[Class Leaf Name] like '%t1%')
	group by AccountId
) NPFC 
...
from (
	select M5dbAccountId AccountId, 
	SUM(case when Category in ( 'Support', 'Support/NOC') and caseType='Other' THEN 1 ELSE 0 END) * coalesce( Max(Com.CostPerSupportCase)/3, 0) CostSupport,
	SUM(case when category in ( 'NOC', 'Support/NOC') and caseType='Other' THEN 1 ELSE 0 END) * coalesce( Max(Com.CostPerNocSupportCase)/3, 0) CostNoc
	 from sfdc.VwSkyCase SC
	 inner join (select * from rollup.CommonStats where  SvcMonth = @svcMonth) Com on 1=1
	 where SC.SfdcCreateDate between @3mBegin and @3mEnd
...
/* 8/22/2016 moved back with 203 etc. 
print 'Costs  210 ...';
--210	Access Ovhd	Access	FCC+Circuits
Insert into rollup.AccountCost
select AccountId, 
	@svcMonth, 210, ( Com.CostAccessOvhdPerCircuit  * A.NumCircuits) Cost -- to change it to new formula
from rollup.TmpAccountStats A
inner join (select * from rollup.CommonStats where  SvcMonth = @svcMonth
	) Com on 1=1
where A.NumCircuits > 0
...
left join commission.VwAcRate PR on PR.AccountId = AR.AccountId and PR.PCMonth = @svcMonth
