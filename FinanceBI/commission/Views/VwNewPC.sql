...
CREATE view [commission].[VwNewPC] as
...
and PC.PCMonth > Intvl.TransitionMonth AND PC.PCMonth >= Intvl.Begin25Month
...
where PC.CreditingPartner is not null 
and PC.PartnerCommissionAmount <> 0.0
and PC.PCMonth <= TransitionMonth and PC.PCMonth >= Begin25Month
...
select 
	'Adjustments' CommissionType -- Adjustment
	,'US'
	 ...
	 ,null PortaAccountId  -->
	  ...
	  ,null City
	  ,null ZipCode
	  ,'USD' PartnerCurrencyCode
	  ,Round([Net Billed],8) NetBilled_PC
      ,Round([Sales Comm],8) SalesComp_PC
