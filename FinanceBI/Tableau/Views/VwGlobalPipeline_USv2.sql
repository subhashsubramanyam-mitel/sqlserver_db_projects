...
CREATE view [Tableau].[VwGlobalPipeline_USv2] as
-- workaround with mat views 
...
SELECT 'US' AS Region
	...
	,CASE WHEN CAST(scon.BusinessTermVersion AS NUMERIC(9, 2)) IS NULL 
		...
		ELSE CAST(scon.BusinessTermVersion AS NUMERIC(9, 2)) 	 END AS BusinessTermVersion
...
LEFT JOIN --oss.VwOrderItemDetail_EX 
		tableau.mVwOrderItem oi ON O.Id = oi.OrderId
...
tableau.mVwsc3 sc3 ON o.Id = sc3.Id
--Direct Linked Contract Info MW 06032020
LEFT JOIN tableau.mVwContract scon ON sc3.OrderSalesContractId = scon.Id
-- MW 12062018  Adds for Kelly
...
LEFT JOIN tableau.mVwMigrationOrderHier mig ON O.Id = mig.OrderId
-- MW 05312019  For Evo Pipeline report
LEFT JOIN tableau.mVwBossPMRole pmr ON pm.Id = pmr.Id
	AND pmr.Instance = 'US'
LEFT JOIN tableau.mVwMasterOrderPM mpm ON O.Id = mpm.OrderId
...
LEFT JOIN tableau.mVwMigCloseOrderDetail mco ON mig.MigrationOrderId = mco.MasterOrderId
