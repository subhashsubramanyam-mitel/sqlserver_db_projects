...
CREATE view [Tableau].[VwRealTimeChurn_Svc] as
...
SELECT T.AccountId
	...
	,[Ac Name]
	,OrderId,ServiceId
	 ,DateEffective,   [OrderStatus], 
		DateSvcClosed,ServiceStatus
...
FROM
	(
 	...
 	SELECT D.[OrderAccountId] AS AccountId
		...
		, D.[DeltaMRR]  As 'MRR'
		,O.OrderId, D.ServiceId
		--,D.DateEffective   
		,O.DateLiveScheduled as  DateEffective
		,O.[OrderStatus], 
		s.DateSvcClosed, ss.Name as ServiceStatus
    ...
    LEFT JOIN  oss.VwServiceProduct_EX s on D.ServiceId = s.ServiceId
	LEFT JOIN enum.ServiceStatus ss on s.ServiceStatusId = ss.Id 
