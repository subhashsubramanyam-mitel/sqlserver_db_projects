...
CREATE VIEW [Tableau].[VwGlobalPipeline_EU]
AS
...
SELECT 'EU' AS Region
	...
	,'EU' + '-' + convert(VARCHAR(10), [VwAccount].[Ac Id]) AS AccountId
	...
	,'EU-' + convert(VARCHAR(15), con.Id) AS ContractId
	,1 MigrationOrderId
	...
	,'EU' + '-' + convert(VARCHAR(10), sc3.OrderSalesContractId) AS OrderSalesContractId
...
LEFT JOIN EU_FinanceBI.enum.ServiceStatus ss(NOLOCK) ON VwServiceProduct_EX.ServiceStatusId = ss.Id
LEFT JOIN EU_FinanceBI.[enum].[VwProduct] ON VwServiceProduct_EX.[ProductId] = [VwProduct].[Prod Id]
LEFT JOIN EU_FinanceBI.oss.[Order] O(NOLOCK) ON VwServiceProduct_EX.OrderId = O.Id
LEFT JOIN EU_FinanceBI.[company].[VwAccount] ON O.[AccountId] = [VwAccount].[Ac Id]
LEFT JOIN EU_FinanceBI.enum.OrderStatus os(NOLOCK) ON O.OrderStatusId = os.Id
LEFT JOIN EU_FinanceBI.oss.[Order] O2(NOLOCK) ON O.MasterOrderId = O2.Id
LEFT JOIN EU_FinanceBI.enum.OrderType ot(NOLOCK) ON o.OrderTypeId = ot.Id
LEFT JOIN EU_FinanceBI.enum.OrderType otm(NOLOCK) ON O2.OrderTypeId = otm.Id
LEFT JOIN EU_FinanceBI.people.VwPerson pm ON VwServiceProduct_EX.OrderProjectManagerId = pm.Id
LEFT JOIN EU_FinanceBI.oss.VwOrderItemDetail oi ON o.Id = oi.OrderId
...
LEFT JOIN (
	...
	FROM EU_FinanceBI.oss.[Order] o(NOLOCK)
	INNER JOIN EU_FinanceBI.oss.[Order] oo(NOLOCK) ON o.LinkedOrderId = oo.Id
...
LEFT JOIN EU_FinanceBI.sales.VwContract scon(NOLOCK) ON sc3.OrderSalesContractId = scon.Id
...
LEFT JOIN EU_FinanceBI.company.VwContractDetail con ON scon.ContractNumber = con.OpportunityNumber
LEFT JOIN tableau.VwBossPMRole pmr ON pm.Id = pmr.Id
	AND pmr.Instance = 'EU'
-- 11182019 For Taskfeed
LEFT JOIN Tableau.mVwTaskfeedInfo tf(NOLOCK) ON 'EU-' + convert(VARCHAR(10), [Loc Id]) = tf.BossLocationId
	AND scon.ContractNumber = tf.OpportunityNumber collate database_default
	AND tf.BossLocationId LIKE 'EU-%'
...
-- Cust Info
LEFT JOIN Tableau.mVwSFDCCustInfo ci(NOLOCK) ON 'EU-' + convert(VARCHAR(10), VwAccount.[Ac Id]) = ci.AccountId collate database_default
	AND isnumeric(ci.AccountId) = 0
	AND ci.AccountId LIKE 'EU-%'
-- MW 02252020  Need to know if account is partner
LEFT JOIN EU_FinanceBI.company.Partner prtnr(NOLOCK) ON VwAccount.[Ac Id] = prtnr.AccountId
LEFT JOIN EU_FinanceBI.oss.ServiceSettings ss2(NOLOCK) ON VwServiceProduct_EX.ServiceId = ss2.ServiceId
LEFT JOIN EU_FinanceBI.oss.ServiceParentProfile spp(NOLOCK) ON spp.ServiceId = VwServiceProduct_EX.ServiceId
