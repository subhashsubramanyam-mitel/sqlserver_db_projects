...
--3 min 38 secs
-- EXEC  [Tableau].[UspInitPipeline_intl_lite]
CREATE PROCEDURE [Tableau].[UspInitPipeline_intl_lite] 
AS
BEGIN
	...
	SET NOCOUNT ON;
		...
		BEGIN
			...
			TRUNCATE TABLE [Tableau].[mVwGlobalPipeline_AU];
		
			INSERT INTO [Tableau].[mVwGlobalPipeline_AU]
				   ([Region]
				   ,[Ac Name]
				   ,[IsBillable]
				   ,[Ac Team]
				   ...
				   ,[Total #s to LNP]
				   ,[TF_MDBoard_Mod_Date]
				   ,[TF_MDBoard_Delay_Reason]
				   ,[TF_MDBoard_Notes]
				   ,[TF_MDBoard_Delay_Description]
				   ,[TF_MDBoard_Project_Number]
				   ,[TF_MDBoard_SFDCID]
				   ,[Debooked Opportunity]
				   ,[PRA Score]
				   ,[Parent Profile])
			 SELECT
				   [Region]
			...
			, Msg)
			SELECT  GETDATE()
			, 'Loaded Tableau.mVwGlobalPipeline_AU for Pipeline';
		...
		BEGIN
			...
			TRUNCATE TABLE [Tableau].[mVwGlobalPipeline_EU];
		
			INSERT INTO [Tableau].[mVwGlobalPipeline_EU]
				   ([Region]
				   ,[Ac Name]
				   ,[IsBillable]
				   ,[Ac Team]
			...
			, Msg)
			SELECT  GETDATE()
			, 'Loaded Tableau.mVwGlobalPipeline_EU for Pipeline';
