...
--
-- Change Log:  2018-03-05 including Connect
-- ================================================================================================================================

CREATE PROCEDURE [cdr].[UspComputeSummaryForUsageCube_New] 
...
where CR.internal=0 and CR.start >= @ServiceMonth and CR.start < @InvoiceMonth and coalesce(TN,'') <> ''
...
select --top 100
  ...
  Right(Tn,10) TN,
  calltypeId CdrCallTypeid,
  coalesce(servicetypeid,1) CdrServiceTypeId,
  routetypeid CdrRegionTypeId,
  COUNT(1) NumCalls,
  SUM(duration/60.0) Minutes,
  SUM(ChargeAmount/100.0) Charge
 
  from  CallData.cdr.Connect_CdrCurrentMonth_InProcess CR
  left join company.Account A on A.Id = CR.Accountid
  left join company.Location L on L.Id = CR.LocationId

where  CR.TimeStart >= @ServiceMonth and CR.TimeStart < @InvoiceMonth and coalesce(TN,'') <> ''
groUP by A.Id, A.LichenAccountId, L.Id, L.LichenLocationId, tn, servicetypeid, routetypeid, calltypeiD
