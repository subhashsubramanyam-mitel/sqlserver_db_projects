...
CREATE view archive.[oss_VwAddOrderService_Sep2015] as
select 
 ...
 OI.ProductId, OI.NewMRR  MRR
from oss.VwOrderItemDetail OI
inner join oss.VwOrder O on O.OrderId = OI.OrderId
left join oss.ServiceProduct S on S.ServiceId = OI.ServiceId and coalesce(S.ProductId,0) = coalesce(OI.ProductId,0)
